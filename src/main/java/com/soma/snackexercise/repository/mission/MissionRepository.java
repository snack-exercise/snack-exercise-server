...
public interface MissionRepository extends JpaRepository<Mission, Long> {
    ...
    @Query("SELECT count(*) AS cnt" +
            ...
            "    ORDER BY cnt DESC" +
            "    LIMIT 1")
    Integer findCurrentFinishedRelayCountByGroupId(@Param("groupId") Long groupId, @Param("startDateTime") LocalDateTime startDateTime, @Param("endDateTime") LocalDateTime endDateTime);
    ...
    @Query("SELECT m" +
            ...
            "   WHERE m.group.id = :groupId AND :startDateTime <= m.createdAt AND m.createdAt < :endDateTime" +
            "   ORDER BY m.createdAt")

    List<Mission> findMissionsByGroupIdWithinDateRange(@Param("groupId") Long groupId, @Param("startDateTime") LocalDateTime startDateTime, @Param("endDateTime") LocalDateTime endDateTime);
